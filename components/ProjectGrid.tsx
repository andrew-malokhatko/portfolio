...
const ProjectGrid = ({ limit }: ProjectGridProps) => {
  ...
  return (
    <div className="w-full grid grid-cols-3 gap-3">
        {projectsToDisplay.map(({name, description, image, technologies, link}) => (
            <div key={name} className="relative p-5 w-full flex flex-col justify-between gap-0.5 border border-border-dark rounded-md duration-500">
